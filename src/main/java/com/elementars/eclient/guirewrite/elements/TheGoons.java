...
public class TheGoons extends Element {
    ...
    public void onRender() {
        ...
        if (Xulu.CustomFont) {
            Xulu.cFontRenderer.drawStringWithShadow("Agalar", (float) x + 1, yCount + 1, ColorUtils.changeAlpha(color, Global.hudAlpha.getValue()));
        } else {
            Wrapper.getMinecraft().fontRenderer.drawStringWithShadow("Agalar", (float) x + 1, yCount + 1, ColorUtils.changeAlpha(color, Global.hudAlpha.getValue()));
        }
        yCount += 10;
